...
export default function PaginaChecklist() {
  ...
  // "Firma" leggera (15/08/2026, modulo 6.1 punto 3): il dato user_id/nome/
  // cognome era già restituito da GET /hr/haccp (JOIN su users), ma mai
  // mostrato — l'intera checklist del giorno è salvata sotto un solo
  // user_id (chi preme "Salva"), quindi basta la prima voce con un nome.
  const compilataDa = voci.find(v => v.nome)?.nome
    ? `${voci.find(v => v.nome).nome} ${voci.find(v => v.nome).cognome}`
    : null;

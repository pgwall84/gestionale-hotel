...
// Cameriere: tocca tavolo libero → apre comanda / tavolo occupato → gestisce comanda.
// Associazione: clicca "Assegna" su una prenotazione → scegli il tavolo dalla griglia.
...
function CardTavolo({ tavolo, modalitaAssegna, onAssegna, onLibero, onOccupato, aprendoId }) {
  ...
  if (modalitaAssegna) {
    // In modalità assegna: verde se libero (selezionabile), grigio se occupato
    ...
    // Prenotazione assegnata ma tavolo ancora libero
    ...
    >
      ...
      {tavolo.etichetta && (
        <span className="text-xs font-semibold truncate" style={{ color: labelColor }}>
          {tavolo.etichetta}
          {tavolo.prenotazione_ora && ` ${tavolo.prenotazione_ora.slice(0,5)}`}
        </span>
      )}
...
export default function SalaPage() {
  ...
  // Modalità assegnazione: prenotazione selezionata in attesa di tavolo
  ...
  const apriComanda = async (tavolo) => {
    setAprendoId(tavolo.id);
    try {
      await api.post('/ristorante/comande', { tavolo_id: tavolo.id });
      await carica();
    } catch (err) {
      if (err.response?.status === 409) await carica();
  ...
  const vaiAComanda = (tavolo) => {
    window.location.href = `/ristorante?comanda=${tavolo.comanda_id}`;
  };

  // Assegna prenotazione selezionata al tavolo cliccato
  const assegnaTavolo = async (tavolo) => {
    if (!prenotazioneInAssegna) return;
    try {
      await api.patch(`/ristorante/tavoli/${tavolo.id}/prenotazione`, {
        prenotazione_id: prenotazioneInAssegna.id,
      });
  ...
  // Rimuovi associazione prenotazione da un tavolo
  ...
  const attivaConfig = async (id) => {
    try {
      await api.patch(`/ristorante/config/${id}/attiva`);
      await carica();
  ...
  const creaConfig = async (e) => {
    ...
    setSalvando(true);
    try {
      await api.post('/ristorante/config', { nome: nuovoNomeConfig.trim() });
      setNuovoNomeConfig('');
      await carica();
    } catch (err) { alert(err.message); }
  ...
  const creaTavolo = async (e) => {
    ...
    setSalvando(true);
    try {
      await api.post('/ristorante/tavoli', {
        numero: parseInt(formTavolo.numero),
        coperti: parseInt(formTavolo.coperti),
      });
  ...
  const eliminaTavolo = async (id) => {
    ...
    try {
      await api.delete(`/ristorante/tavoli/${id}`);
      await carica();
    } catch (err) { alert(err.message); }
  };
  ...
  // Prenotazioni con e senza tavolo assegnato
      ...
      <div className="p-4 flex flex-col gap-4 max-w-xl mx-auto">
        ...
        <div className="flex justify-between items-center">
          ...
          {isAdmin && (
            <button
              ...
              style={{
                background: pannelloAperto ? 'var(--primary)' : 'var(--muted)',
                color: pannelloAperto ? 'var(--primary-foreground)' : 'var(--foreground)',
              }}
            >
        ...
        {prenotazioneInAssegna && (
          <div className="rounded-xl px-4 py-3 flex justify-between items-center"
            ...
            <div>
              <p className="text-sm font-bold" style={{ color: 'var(--status-blue-text)' }}>
                Tocca un tavolo libero per assegnarlo a:
              </p>
              <p className="text-sm" style={{ color: 'var(--foreground)' }}>
                {prenotazioneInAssegna.nome} — ore {prenotazioneInAssegna.ora?.slice(0,5)} ({prenotazioneInAssegna.coperti} pers.)
              </p>
            </div>
            <button onClick={() => setPrenotazioneInAssegna(null)}
            ...
            <div className="flex" style={{ borderBottom: '1px solid var(--border)' }}>
              {[{ id: 'tavoli', label: 'Tavoli' }, { id: 'config', label: 'Layout sala' }].map(t => (
                <button key={t.id} onClick={() => setSezioneAttiva(t.id)}
                        ...
                        style={{
                          background: sezioneAttiva === t.id ? 'var(--primary)' : 'var(--card)',
                          color: sezioneAttiva === t.id ? 'var(--primary-foreground)' : 'var(--muted-foreground)',
                        }}>
                  {t.label}
                </button>
              ))}
        ...
        {/* Legenda (solo quando non in modalità assegna) */}
              ...
              {prenotazioni.map(p => {
                ...
                return (
                  <div key={p.id}
                       ...
                       style={{
                         background: assegnata ? 'var(--status-blue-bg)' : 'var(--card)',
                         border: `1px solid ${assegnata ? 'var(--status-blue-text)' : 'var(--border)'}`,
                       }}>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>
                        ...
                        {tavoloAssegnato && (
                          <span style={{ color: 'var(--status-blue-text)' }}> · T{tavoloAssegnato.numero}</span>
                        )}
                      </p>
                    </div>
                    ...
                    {puoAssegnare && (
                      assegnata ? (
                        <button
                          onClick={() => rimuoviAssociazione(tavoloAssegnato.id)}
                        ...
                        <button
                          onClick={() => setPrenotazioneInAssegna(p)}
